...
public class ChatMatchesAdapter extends BaseAdapter {
	...
	public View getView(int position, View rowView, ViewGroup parent) {
		...
		if (rowView == null) {
			...
			} else if (check == 1) { // chat edit
				rowView = ((Activity) mActivity).getLayoutInflater().inflate(
						R.layout.row_home4, null);
				viewHolder.userimage = (CircleImageView) rowView
						.findViewById(R.id.userimage);
			} else if (check == 2) { // favorites grid view
				rowView = ((Activity) mActivity).getLayoutInflater().inflate(
			...
			// Log.e("heyyy", userDTO.getUnreadMsg());
			// Log.e("heyyy id ", userDTO.getFirstName());
		...
		// setUserChatInformation(userDTO);

		if (check != 2) {
			if (userDTO.getUserProfileImageDTOs().size() != 0
					&& userDTO.getUserProfileImageDTOs() != null) {

				if (userDTO.getUserProfileImageDTOs().get(0).getIsImageActive() == true) {
					...
					Log.d("diofel", "nakasulod checker = o yes ");
				} else {
					// edited on revision 594
					checker = "no";
					// - - - - -
					...
					Log.d("diofel", "nakasulod checker = o no");
				}

			} else if (check == 3 || check == 1) {
				// edited on revision 594
				// userDTO.getProfileImage() to AppConstants.AWATAR_IAMGE
				...
				imageLoaderCircle.DisplayImage(AppConstants.AWATAR_IAMGE,
						viewHolder.userimage);
				Log.d("diofel", "nakasulod checker = no1");
			}
		} else {
			try {
				if (userDTO.getUserProfileImageDTOs().size() != 0
					...
					if (userDTO.getUserProfileImageDTOs().get(0)
						...
						Log.d("diofel", "nakasulod checker = no2");
		...
		if (check == 3 || check == 1) {
			/* Unread Chat Count */
			try {
				if (singleton.getChatCount.get(userDTO.getUserID()) > 0) {// Integer.valueOf(userDTO.getUnreadMsg())
																			// >
																			// 0)
																			// {
		...
		if (check == 3 || check == 1) {
			...
			try {
				if (userDTO.getUserLastMsg().contains("|")) {
					viewHolder.email.setText(userDTO.getUserLastMsg().split(
							"\\|")[2]);
				} else {
				...
				if (userDTO.getUserLastMsgFromOrTo().equals(
						Activity_Home.sessionManager.getUserDetail()
								.getUserID())) {
				...
				viewHolder.lastMsgTime.setText(getTime(userDTO
						.getUserLastMsgTime()));
		...
		// viewHolder.email.setVisibility(View.VISIBLE);
		// viewHolder.email.setText(userDTO.getUserLastMsg());
		// // viewHolder.email.setText(filteredData.get(position).get);
		//
		// //
